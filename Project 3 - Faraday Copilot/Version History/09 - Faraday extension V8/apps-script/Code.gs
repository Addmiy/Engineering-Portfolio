function doGet(e) {
  try {
    ...
    const documentId = String(e.parameter.documentId || e.parameter.docId || '').trim();
    const inferredAction = String(e.parameter.action || (e.parameter.docId ? 'snapshot' : 'snapshot'));
    if (!documentId) return jsonOut({ ok: false, error: 'Missing documentId or docId' });
    if (inferredAction !== 'snapshot') return jsonOut({ ok: false, error: 'Unsupported action' });
