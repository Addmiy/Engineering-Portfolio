...
async function fetchCompanionDocSnapshot(documentId, settings) {
  ...
  const res = await fetch(url.toString(), { credentials: 'omit', cache: 'no-store' });
  if (!res.ok) throw new Error(`Companion endpoint failed (${res.status})`);
  const data = await res.json();
  if (!data?.ok) throw new Error(data?.error || 'Companion endpoint returned an invalid payload');
  return {
    text: cleanDocsText(data.text || ''),
    title: data.title || '',
...
async function getGoogleDocsContext({ forceRefresh = false } = {}) {
  ...
  const bridgeText = cleanDocsText(window.SeamlessAIDocsBridge?.getLikelyDocumentText?.() || '');
  ...
  let companion = null;
  try {
    companion = await fetchCompanionDocSnapshot(documentId, settings);
  } catch (error) {
    console.debug('Faraday Docs companion unavailable:', error?.message || error);
  }

  let exportedText = '';
  try {
    exportedText = await fetchGoogleDocsExportText(documentId);
  } catch (error) {
    console.debug('Faraday Docs export fallback unavailable:', error?.message || error);
  }

  const candidates = [
    companion && { ...companion, score: scoreDocsText(companion.text) },
    exportedText && { text: exportedText, title: bridgeTitle, headings: [], lists: [], tables: [], source: 'Google Docs export', exportedAt: new Date().toISOString(), selection: bridgeSelection, score: scoreDocsText(exportedText) },
    bridgeText && { text: bridgeText, title: bridgeTitle, headings: [], lists: [], tables: [], source: 'DOM bridge fallback', exportedAt: new Date().toISOString(), selection: bridgeSelection, score: scoreDocsText(bridgeText) }
  ...
  const value = {
    documentId,
    text: cleanDocsText(best.text),
    title: best.title || bridgeTitle,
    headings: best.headings || [],
    lists: best.lists || [],
    tables: best.tables || [],
    source: best.source,
    exportedAt: best.exportedAt || new Date().toISOString(),
    selection: best.selection || bridgeSelection
  };
