(() => {
  ...
  function getSelectedText() {
    try {
      return normalizeBlock(window.getSelection()?.toString() || '');
    } catch {
      return '';
    }
  }
  ...
  window.SeamlessAIDocsBridge = {
    ...
    getSelectedText,
    getDocumentTitle() {
      const title = document.querySelector('.docs-title-input-label-inner, .docs-title-input')?.textContent || document.title || '';
      return normalizeLine(title.replace(/\s*-\s*Google Docs\s*$/i, ''));
    }
  };
})();
