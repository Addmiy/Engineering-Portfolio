...
const assistantIdentity = document.getElementById('assistantIdentity');
const modelSelect = document.getElementById('modelSelect');
const modelMeta = document.getElementById('modelMeta');
const modelCards = document.getElementById('modelCards');
const personalitySelect = document.getElementById('personalitySelect');
const pageContextToggleBtn = document.getElementById('pageContextToggleBtn');
...
const expandedTools = document.getElementById('expandedTools');
const uploadImageBtn = document.getElementById('uploadImageBtn');
const utilityDrawer = document.getElementById('utilityDrawer');
const utilityTabs = Array.from(document.querySelectorAll('.utility-tab'));
const utilityPanels = Array.from(document.querySelectorAll('[data-utility-panel]'));
const closeUtilityDrawerBtn = document.getElementById('closeUtilityDrawerBtn');
...
const openFilesFooterBtn = document.getElementById('openFilesFooterBtn');
const openCitationsFooterBtn = document.getElementById('openCitationsFooterBtn');
...
const FULL_APP = document.body.classList.contains('full-app');
...
let currentView = urlParams.get('view') || (FULL_APP ? 'graph' : 'chat');
let activeUtilitySection = 'page';
let responseLayoutMode = 'structured';
...
async function init() {
  ...
  currentView = settings.activePanelView || currentView;
  if (!['chat', 'graph', 'model'].includes(currentView)) currentView = 'chat';
  ...
  if (FULL_APP) {
    ...
    expandGraphBtn.disabled = true;
    expandGraphBtn.textContent = 'Already fullscreen';
  }
...
function attachEvents() {
  ...
  pageContextToggleBtn?.addEventListener('click', () => openUtilityDrawer('page'));
  contextPinBtn?.addEventListener('click', toggleContextPin);
  ...
  clearContextBtn.addEventListener('click', async () => {
    ...
    settings = (await chrome.runtime.sendMessage({ type: 'SAVE_SETTINGS', partial: { lastPageContext: null, pinnedPageContext: null, contextPinned: false } })).settings;
    syncUiState();
  });
  ...
  openFullscreenBtn?.addEventListener('click', async () => {
    const response = await chrome.runtime.sendMessage({ type: 'OPEN_FULLSCREEN_APP' });
    if (!response?.ok) appendMessage({ role: 'assistant', text: `Could not open fullscreen tab: ${response?.error || 'Unknown error'}` });
  });

  moreToolsBtn?.addEventListener('click', () => expandedTools?.classList.toggle('hidden'));
  ...
  layoutModeBtn?.addEventListener('click', cycleLayoutMode);
  ...
  splitViewBtn?.addEventListener('click', () => setActiveView('graph'));
  ...
  closeUtilityDrawerBtn?.addEventListener('click', () => utilityDrawer?.classList.add('hidden'));
  utilityTabs.forEach((button) => button.addEventListener('click', () => openUtilityDrawer(button.dataset.utility)));
  openFilesFooterBtn?.addEventListener('click', () => openUtilityDrawer('files'));
  openCitationsFooterBtn?.addEventListener('click', () => openUtilityDrawer('citations'));
  settingsBtn?.addEventListener('click', () => chrome.runtime.openOptionsPage());
  helpBtn?.addEventListener('click', async () => {
    await chrome.tabs.create({ url: chrome.runtime.getURL('README.md') }).catch(() => chrome.runtime.openOptionsPage());
  });
  upgradeBtn?.addEventListener('click', () => appendMessage({ role: 'assistant', text: 'The upgrade button is wired as a placeholder. Hook it to your billing page when you are ready to harvest subscriptions from the exhausted masses.' }));
  favoriteBtn?.addEventListener('click', () => {
    settings = { ...settings, favoriteSavedAt: Date.now() };
    updateFooterStats('Conversation saved locally. Tiny act of digital hoarding complete.');
  });
  ...
  expandGraphBtn.addEventListener('click', async () => {
    if (FULL_APP) return;
    await chrome.runtime.sendMessage({ type: 'OPEN_FULLSCREEN_APP', initialView: 'graph' });
  });
  ...
  modelCards?.addEventListener('click', async (event) => {
    const target = event.target.closest('[data-model-id]');
    if (!target) return;
    modelSelect.value = target.dataset.modelId;
    settings = (await chrome.runtime.sendMessage({ type: 'SAVE_SETTINGS', partial: { selectedModel: modelSelect.value } })).settings;
    updateModelMeta();
    renderModelCards();
  });
  ...
  window.addEventListener('resize', debounce(() => {
    if (graphState.functions.length) renderGraph();
  }, 60));
}
...
function setActiveView(view, persist = true) {
  currentView = ['chat', 'graph', 'model'].includes(view) ? view : 'chat';
  ...
  if (persist) chrome.runtime.sendMessage({ type: 'SAVE_SETTINGS', partial: { activePanelView: currentView } }).catch(() => null);
  if (currentView === 'graph' && graphState.functions.length) requestAnimationFrame(renderGraph);
  syncUiState();
}
...
function renderChat(messages) {
  chatLog.innerHTML = '';
  if (!messages.length) {
    chatLog.innerHTML = '<div class="message assistant"><div class="message-content"><p>No conversation yet. That giant empty box is doing its best impression of productivity.</p></div></div>';
    updateFooterStats();
    return;
  }
...
function renderModelCards() {
  ...
  modelCards.innerHTML = models.map((model) => {
    const active = model.id === modelSelect.value;
    return `
      <article class="model-chip ${active ? 'active-state' : ''}">
        <h3>${escapeHtml(model.label)}</h3>
        <p>${escapeHtml(model.notes || 'Model option')}</p>
        <div class="chip-meta">Speed: ${escapeHtml(model.speed || 'Unknown')}<br />Reasoning: ${escapeHtml(model.reasoning || 'Unknown')}<br />Context: ${escapeHtml(model.context || 'Unknown')}</div>
        <button type="button" data-model-id="${escapeHtml(model.id)}" class="${active ? 'active-state' : 'ghost'}">${active ? 'Selected' : 'Use model'}</button>
      </article>
    `;
  }).join('');
}
...
async function refreshPageContext(preferredPageContext = null) {
  ...
  if (settings?.contextPinned && settings?.pinnedPageContext) {
    currentPageContext = settings.pinnedPageContext;
    renderPageContextMeta();
    return;
  }
...
function renderPageContextMeta() {
  ...
  pageContextMeta.innerHTML = `
    <div><strong>Title:</strong> ${escapeHtml(currentPageContext.title || 'Untitled')} ${settings?.contextPinned ? '<span style="color:#7fb0ff">• pinned</span>' : ''}</div>
...
async function submitComposer() {
  ...
  const finalPrompt = buildPromptWithModes(prompt);
  await sendPrompt(finalPrompt, { displayPrompt: prompt });
  chatInput.value = '';
}
...
async function sendPrompt(prompt, extra = {}) {
  appendMessage({ role: 'user', text: extra.displayPrompt || prompt });
  flashThinking('Thinking…');
...
function setGreeting() {
  ...
  if (greetingTitle) greetingTitle.textContent = `Hi, good ${period}!`;
  if (assistantIdentity) assistantIdentity.textContent = `I'm Faraday, your AI copilot.`;
}

function syncUiState() {
  contextPinBtn?.classList.toggle('active-state', Boolean(settings?.contextPinned));
  pageContextToggleBtn?.classList.toggle('active-state', !utilityDrawer?.classList.contains('hidden') && activeUtilitySection === 'page');
  webToggleBtn?.classList.toggle('active-state', webModeEnabled);
  designToggleBtn?.classList.toggle('active-state', designModeEnabled);
  layoutModeBtn?.classList.toggle('active-state', responseLayoutMode !== 'structured');
  if (layoutModeBtn) layoutModeBtn.title = `Response layout: ${responseLayoutMode}`;
}

function openUtilityDrawer(section = 'page') {
  activeUtilitySection = section;
  utilityDrawer?.classList.remove('hidden');
  utilityTabs.forEach((button) => button.classList.toggle('active', button.dataset.utility === section));
  utilityPanels.forEach((panel) => panel.classList.toggle('active', panel.dataset.utilityPanel === section));
  syncUiState();
}

async function toggleContextPin() {
  const nextPinned = !settings?.contextPinned;
  settings = (await chrome.runtime.sendMessage({
    type: 'SAVE_SETTINGS',
    partial: {
      contextPinned: nextPinned,
      pinnedPageContext: nextPinned ? currentPageContext : null
    }
  })).settings;
  syncUiState();
  renderPageContextMeta();
...
function cycleLayoutMode() {
  const order = ['concise', 'structured', 'deep analysis'];
  const index = order.indexOf(responseLayoutMode);
  responseLayoutMode = order[(index + 1) % order.length];
  syncUiState();
  flashThinking(responseLayoutMode);
}

function buildPromptWithModes(prompt) {
  const instructions = [];
  if (responseLayoutMode === 'concise') instructions.push('Respond concisely but still clearly.');
  if (responseLayoutMode === 'deep analysis') instructions.push('Respond with deep analysis, thorough structure, and explicit reasoning steps.');
  if (webModeEnabled) instructions.push('Use any available web or page context awareness where relevant and mention sources when possible.');
  if (designModeEnabled) instructions.push('Prioritise visual thinking, layout suggestions, interface ideas, and design-oriented recommendations where relevant.');
...
function enhanceCurrentPrompt() {
  const prompt = chatInput.value.trim();
  if (!prompt) return;
  const personalityLabel = personalitySelect.options[personalitySelect.selectedIndex]?.text || 'assistant';
  const layoutInstruction = responseLayoutMode === 'deep analysis' ? 'with deep analysis and clear sections' : responseLayoutMode === 'concise' ? 'briefly and clearly' : 'with structured headings and bullet points';
  chatInput.value = `As a ${personalityLabel.toLowerCase()}, answer this ${layoutInstruction}: ${prompt}`;
  chatInput.focus();
}

function flashThinking(text) {
  if (!thinkingIndicator) return;
  thinkingIndicator.textContent = `✦ ${text}`;
}

function updateFooterStats(message = null) {
  if (!tokenCounter) return;
  const history = settings?.chatHistory || [];
  const totalTokens = history.reduce((sum, item) => sum + Number(item?.usage?.total_tokens || 0), 0);
  tokenCounter.textContent = `⚡ ${totalTokens || 0} tokens`;
  if (message) flashThinking(message);
}

function startVoiceInput() {
  const Recognition = window.SpeechRecognition || window.webkitSpeechRecognition;
  if (!Recognition) {
    appendMessage({ role: 'assistant', text: 'Voice input is not supported in this browser build. Chrome giveth, Chrome randomly withholdeth.' });
    return;
  }
  ...
  recognition.onstart = () => flashThinking('Listening');
  recognition.onerror = () => flashThinking('Ready');
  recognition.onend = () => flashThinking('Ready');
  recognition.onresult = (event) => {
    const transcript = event.results?.[0]?.[0]?.transcript || '';
    chatInput.value = [chatInput.value.trim(), transcript].filter(Boolean).join(chatInput.value.trim() ? ' ' : '');
  };
  recognition.start();
}
...
function renderGraph() {
  ...
  const cssWidth = Math.max(320, Math.floor(bounds.width || graphCanvas.clientWidth || 320));
  const cssHeight = Math.max(FULL_APP ? 540 : 360, Math.floor(bounds.height || graphCanvas.clientHeight || (FULL_APP ? 540 : 360)));
  ...
  for (const fnInfo of graphState.functions) {
    drawFunction(ctx, width, height, fnInfo, toX, toY, xMin, xMax, yMin, yMax);
  }

  drawMarkers(ctx, toX, toY, graphState.markers);

  if (graphState.mouseX != null && graphState.mouseY != null && graphState.functions.length) {
    const hoverText = drawHoverInfo(ctx, toX, toY, width, xMin, xMax);
    if (hoverText) {
  ...
  const zoomMode = graphState.allowWheelZoom ? 'wheel zoom enabled' : 'fixed zoom in side panel';
  graphMeta.textContent = `${graphState.functions.map((item) => item.label).join(' • ')} • x:[${xMin.toFixed(2)}, ${xMax.toFixed(2)}] • y:[${yMin.toFixed(2)}, ${yMax.toFixed(2)}] • drag to pan • ${zoomMode}`;
}
...
function drawFunction(ctx, width, height, fnInfo, toX, toY, xMin, xMax, yMin, yMax) {
  ...
  for (let px = 0; px < width; px += 1) {
    ...
    const py = toY(y);
    const verticalJump = previousY != null && Math.abs(py - previousY) > height * 0.45;
...
function drawMarkers(ctx, toX, toY, markers) {
  for (const marker of markers) {
    ...
    ctx.lineWidth = 1.5;
    ctx.beginPath();
    ctx.arc(px, py, marker.kind === 'stationary' ? 6.5 : 5.5, 0, Math.PI * 2);
    ctx.fill();
    ctx.stroke();
  }
}
...
function drawGrid(ctx, width, height, xMin, xMax, yMin, yMax, toX, toY) {
  ...
  ctx.font = `${Math.max(18, Math.floor(width * 0.014))}px Inter, Arial, sans-serif`;
