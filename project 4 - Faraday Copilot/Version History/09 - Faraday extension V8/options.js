...
const fields = {
  ...
  docsCompanionUrl: document.getElementById('docsCompanionUrl')
};
...
function hydrateFields() {
  ...
  fields.docsCompanionUrl.value = settings.docsCompanionUrl || '';
  renderCustomModels();
...
async function persist() {
  ...
  const normalizedDocsUrl = normalizeDocsCompanionUrl(fields.docsCompanionUrl.value);
  fields.docsCompanionUrl.value = normalizedDocsUrl;
  ...
  const partial = {
    ...
    docsCompanionUrl: normalizedDocsUrl,
  ...
  try {
    ...
    if (!response?.settings) {
      throw new Error('Settings save failed. No response came back from the extension.');
    }
    settings = response.settings;
    renderModelOptions(response.models || DEFAULT_MODELS);
    renderPersonalityOptions(settings.personalities || DEFAULT_PERSONALITIES);
    ...
    status.textContent = normalizedDocsUrl
      ? 'Saved. Google Docs companion URL stored.'
      : 'Saved.';
  } catch (error) {
    console.error(error);
...
function normalizeDocsCompanionUrl(value) {
  ...
  try {
    ...
    if (!url.pathname.endsWith('/exec')) {
      return raw.replace(/\?docId=.*$/i, '').trim();
    }
