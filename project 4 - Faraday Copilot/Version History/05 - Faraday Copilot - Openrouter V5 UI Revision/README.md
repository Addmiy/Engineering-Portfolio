...
## Google Docs companion compatibility

The extension now accepts both companion styles: a beginner Apps Script that expects `?docId=...` and the richer snapshot companion that expects `?action=snapshot&documentId=...`. You still paste only the base `/exec` URL into settings.


## Google Docs companion diagnostics

This build fetches the Apps Script companion through the extension background service worker instead of directly from the content script. That avoids the cross-origin fetch failure path that can happen on Google Docs pages under Manifest V3. If the companion redirects to Google sign-in or returns non-JSON HTML, Faraday now surfaces a specific setup error instead of the vague `Failed to fetch` message.
